              ...
              <section aria-label="Rating breakdown">
                <div className="flex items-start gap-5 bg-surface rounded-card shadow-card p-5">
                  <div className="text-center shrink-0">
                    ...
                    <div className="text-[12px] text-primary-light mt-1">out of 10</div>
                  </div>
...
function RatingForm({ cafeId, userId, onClose, onSave }: RatingFormProps) {
  ...
  return (
    <div
      ...
      role="dialog" aria-modal="true" aria-label="Rate an item"
    >
      <div className="bg-surface w-full max-w-lg rounded-t-3xl sm:rounded-card p-6 pb-safe">
        <div className="w-9 h-1 bg-neutral-dark rounded-full mx-auto mb-4 sm:hidden" />
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-[20px] font-bold text-primary">Rate an item</h2>
          <button type="button" onClick={onClose} aria-label="Close"
            className="w-11 h-11 flex items-center justify-center rounded-full bg-neutral text-primary-light active:opacity-60">
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="mb-4">
            <button type="button" onClick={() => fileInputRef.current?.click()}
              className="w-full h-28 rounded-xl bg-neutral border-2 border-dashed border-neutral-dark flex flex-col items-center justify-center gap-2 active:opacity-60 overflow-hidden"
              ...
              {photo ? (
                ...
                <>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-7 h-7 text-primary-light">
            ...
            <input ref={fileInputRef} type="file" accept="image/*" capture="environment"
              onChange={handlePhotoChange} className="sr-only" tabIndex={-1} />
          </div>
          <div className="mb-4">
            <label htmlFor="item-name" className="block text-[13px] font-semibold text-primary-light uppercase tracking-wide mb-2 px-1">
              Item name <span className="text-ios-red">*</span>
            </label>
            <input id="item-name" type="text" value={itemName}
              onChange={(e) => { setItemName(e.target.value); setError('') }}
              placeholder="e.g. Flat White, Croissant…"
              className="w-full px-4 py-3.5 rounded-xl text-[17px] text-primary bg-neutral focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 placeholder:text-primary-light/50"
              required autoFocus />
          </div>
          <div className="mb-5">
            <div className="flex items-center justify-between mb-3 px-1">
              <label htmlFor="score-slider" className="text-[13px] font-semibold text-primary-light uppercase tracking-wide">Score</label>
            ...
            <input id="score-slider" type="range" min={1} max={10} step={1} value={score}
          ...
          <div className="mb-5">
            <label htmlFor="rating-notes" className="block text-[13px] font-semibold text-primary-light uppercase tracking-wide mb-2 px-1">
              Notes <span className="font-normal normal-case text-primary-light">(optional)</span>
            </label>
            <textarea id="rating-notes" value={notes} onChange={(e) => setNotes(e.target.value)}
              placeholder="Any thoughts on this item…" rows={3}
              className="w-full px-4 py-3.5 rounded-xl text-[17px] text-primary bg-neutral focus:outline-none focus-visible:ring-2 focus-visible:ring-accent/40 placeholder:text-primary-light/50 resize-none" />
          </div>
